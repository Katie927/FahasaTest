...
<link rel="stylesheet"
	href="${classpath}/backend/css/ttest.css">
<!--
			...
			<div class="collapse navbar-collapse" id="navbarSupportedContent">
				<ul class="navbar-nav mx-auto h-100">
					...
					<li class="nav-item"><a class="nav-link"
						href="${classpath }/admin/product/list"> <i class="fas fa-shopping-cart"></i>
							Products
					</a></li>
							...
							<tbody>
								...
								<c:forEach var="publisher" items="${publishers }" varStatus="loop">
									<tr>
										...
										<td class="tm-product-name" data-category-id="${publisher.id }">${publisher.id }</td>
										<td class="tm-product-name" data-category-id="${publisher.id }">${publisher.name }</td>
										<td>${publisher.name }</td>
										<td>${publisher.userCreatePublisher.username } -/-
											${publisher.userUpdatePublisher.username }</td>
										<td><fmt:formatDate value="${publisher.createDate }"
												pattern="dd-MM-yyyy" /> -/- <fmt:formatDate
												value="${publisher.updateDate }" pattern="dd-MM-yyyy" /></td>
										<td><span id="_product_status_${publisher.id }"> <c:choose>
													<c:when test="${publisher.status }">
														<span>Active</span>
													</c:when>
													<c:otherwise>
														<span>Inactive</span>
										...
										<td>
											...
											--> <a
											href="${classpath }/admin/publisher/delete/${publisher.id }">
...
-->
					...
					<!-- table container -->
					<a href="${classpath }/admin/publisher/add"
						class="btn btn-primary btn-block text-uppercase mb-3">Add
						publisher</a>
	...
	<script>
		$(function() {
			$(".tm-product-name").on("click", function() {
				...
				window.location.href = "${classpath}/admin/publisher/edit/" + productId;
			});
		});
	</script>
</body>
